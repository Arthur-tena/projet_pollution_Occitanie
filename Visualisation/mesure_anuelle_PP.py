...
data = pd.read_csv(
    "/Users/arthurtena/Documents/data/Mesure_annuelle_Region_Occitanie_Polluants_Principaux.csv"
)
...
for polluant, color in zip(grouped_df["nom_poll"].unique(), colors.colors):
    plt.bar(
        [
            year_indexes[annee] + j * (bar_width + bar_spacing)
            for j, annee in enumerate(grouped_df["date_debut"].unique())
        ],
...
plt.xticks(
    [i for i in range(len(grouped_df["date_debut"].unique()))],
    grouped_df["date_debut"].unique(),
)

...
def create_polar_plot(ville, chemin):
    ...
    df_moyennes_ville = (
        df_ville.groupby(["nom_poll", "mois"])["valeur"].mean().reset_index()
    )
    ...
    fig = px.line_polar(
        df_moyennes_ville,
        r="moyenne_valeur",
        theta=df_moyennes_ville["mois"] * (360 // 12),
        line_close=True,
        color="nom_poll",
        line_dash="nom_poll",
        title="Évolution des polluants par mois de la région",
    )
    liste_des_mois = [
        "Décembre",
        "Janvier",
